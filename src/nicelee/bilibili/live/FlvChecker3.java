...
//		args = new String[] { "D:\\Workspace\\javaweb-springboot\\BilibiliLiveRecord\\download\\周末yuuko-3096485 的bili直播 2019-09-18 15.20-0_flv.flv" };
		args = new String[] {
				"D:\\Workspace\\javaweb-springboot\\BilibiliLiveRecord\\download\\LOL游戏玩家小萌妹-mianf666 的kuaishou直播 2019-09-18 15.52-0_flv.flv" };

		FlvChecker3 fChecker = new FlvChecker3();
...
//			fChecker.checkFromEnd(args[0]);
	 ...
	 * @throws IOException
	 */
	public void deleteMtoNTag(File f, int m, int n) throws IOException {
		RandomAccessFile raf = new RandomAccessFile(f, "r");
		long pointerM = tailOfNthTag(raf, m);
		long pointerN = tailOfNthTag(raf, n);

		raf.seek(pointerM);
		int tagSize = readBytesToInt(raf, 4);
		long minp = pointerM - tagSize;
		long maxp = pointerN + 4;

		File fNew = new File(f.getAbsolutePath() + ".tmp");
		RandomAccessFile rafNew = new RandomAccessFile(fNew, "rw");
		// 0 ~ minp
		raf.seek(0);
		long pointer = 0;
		int readSize = raf.read(buffer);
		while (pointer < minp) {
			rafNew.write(buffer, 0, readSize);
			pointer += readSize;
		...
		rafNew.setLength(rafNew.getFilePointer());
		rafNew.close();
		raf.close();
//		 f.delete();
//		 fNew.renameTo(f);
	 ...
	 * @throws IOException
	 */
	private long tailOfNthTag(RandomAccessFile raf, int num) throws IOException {
		raf.seek(0);
		raf.skipBytes(9);// 头部

		long position = 0;
		while (num > 0) {
			raf.skipBytes(4);// 前一tag长度
			raf.skipBytes(1);// tag类型
			int dataSize = readBytesToInt(raf, 3);// tag data长度
			System.out.println(dataSize);
			raf.skipBytes(4 + 3);// 时间戳+ID
		...
		try {
			...
			while (true) {// && remain>=0
				...
				if (tagType == 8 || tagType == 9) {// 8/9 audio/video
					...
					// 先判断是什么类型：
					int type;
					while (tmp + 3 + 7 + size > raf.getFilePointer()) {
						type = raf.read();
						if (type == 2) {
							System.out.print("String类型：");
							int strLen = readBytesToInt(raf, 2);
							raf.read(buffer, 0, strLen);
							System.out.println(new String(buffer, 0, strLen));
						} else if (type == 3) {
							System.out.print("Obj类型：\n");

							while (tmp + 3 + 7 + size >= raf.getFilePointer()) {
								// 接下来
								// 2 + len String
								int strLen = readBytesToInt(raf, 2);
								System.out.println("Key size:" + strLen);
								raf.read(buffer, 0, strLen);
								System.out.println("Key:" + new String(buffer, 0, strLen));
								// 1 + len value
								type = raf.read();
								if (type == 0) {
									System.out.print("Value: Number类型：");
									byte[] num = new byte[8];
									raf.read(num);
									double valued = bytes2Double(num);
									System.out.println(valued);
								} else if (type == 2) {
									System.out.print("Value: String类型：");
									strLen = readBytesToInt(raf, 2);
									raf.read(buffer, 0, strLen);
									System.out.println(new String(buffer, 0, strLen));
								} else if (type == 9) {
									System.out.print("Obj类型结束\n");
									break;
								} else {
									System.out.print("Value: 未知类型" + type);
									System.exit(1);
								}
							}
						} else if (type == 0) {
							System.out.print("Number类型：");
							byte[] num = new byte[8];
							raf.read(num);
							double valued = bytes2Double(num);
							System.out.println(valued);
							...
							System.out.print("ECMAArray类型：");
							int strLen = readBytesToInt(raf, 4);
							System.out.println("ECMAArray size:" + strLen);
							while (tmp + 3 + 7 + size >= raf.getFilePointer()) {
								// 接下来
								// 2 + len String
								strLen = readBytesToInt(raf, 2);
								...
								if (type == 0) {
									...
									System.out.print("Value: Unit8类型：");
							...
							System.out.println(tmp + 3 + 7 + size);
...
//					}
						...
						raf.seek(pos - 5);
						// 2. 处理新文件
						FlvChecker3 fc = new FlvChecker3();
